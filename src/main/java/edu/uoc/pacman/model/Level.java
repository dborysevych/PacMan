...
public class Level {
    ...
    public MapItem getMapItem(int x, int y) {
        ...
        int index =  x == 0 || y == 0 ? x + y :  y * getWidth() + x;

        return getMapItemList().get(index);
    }
    ...
    public void removeMapItem(MapItem item) {
        ...
        int index = mapItemList.indexOf(item);

        mapItemList.set(index, new Path(position));
    }
    ...
    public boolean isPathable(Position position) {
        return getMapItem(position).isPathable();
    }
